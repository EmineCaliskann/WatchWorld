...
namespace Web.Services
{
    public class HomeViewModelService : IHomeViewModelService
    {
        ...
        public async Task<HomeViewModel> GetHomeViewModelAsync(int? categoryId, int? brandId, int pageId)
        {
            ...
            var specProducts = new CatalogFilterSpesification(categoryId, brandId);
            var totalItems = await _productRepo.CountAsync(specProducts);
            var specProductsPaginated = new CatalogFilterSpesification(categoryId, brandId, (pageId - 1) * Constants.ITEMS_PER_PAGE, Constants.ITEMS_PER_PAGE);
